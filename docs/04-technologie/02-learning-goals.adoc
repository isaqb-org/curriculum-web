...
** Reverse Proxies sind Proxies auf der Server-Seite und können unterschiedliche Aufgaben wahnehmen:
*** Terminierung der TLS Verbindung,
*** Caching der Inhalte der dahinter liegenden Web-Applikationen - zum teil auch aktiv gemanaget (Purge, Cache-Groups),
*** Authentisierung, Autorisierung und andere Security Features (letzteres oft in Form einer Web Application Firewall),
*** Routing, Load Balancing, Throttling, Billing - oft auch in Form eines API-Gateway Produkts.
** Backend for Frontends (BFF) sind keine Reverse Proxies. Es handelt sich um eine serverseitig laufende Komponente, die konzeptionell zur frontendseitigen Anwendung gehört und speziell auf deren Anforderungen zugeschnitten ist.
...
* Die Teilnehmer verstehen, wie mittels Reverse-Proxy die Aufgabe der Authentisierungsschemata (beispielsweise von Form-based auf Basic-Auth.) umgeschrieben werden können.
...
** Reverse proxies are proxies on the server side and can fulfill various tasks:
*** Termination of the TLS connection,
*** Caching of the content of the underlying web applications — in some cases actively managed (purge, Cache Groups),
*** Authentication, authorization, and other security features (the latter often implemented in the form of a Web Application Firewall),
*** Routing, load balancing, throttling, billing — often provided as part of an API gateway product.
** Backend for Frontends (BFFs) are not reverse proxies. They are server-side components that conceptually belong to the frontend application and are tailored to its specific requirements.
