...
**Was sollen die Teilnehmer verstehen?**
  ...
  * Die Teilnehmer verstehen die wesentlichen Eigenschaften des HTTP-Protokolls und können diese erklären.
    ...
    ** Das HTTP-Protokoll bietet Raum für unterschiedliche Verfahren zur Authentisie-rung des Clients gegenüber dem Server (z.B. Basic-Authentication).
  ...
  * Die Teilnehmer verstehen, dass es für unterschiedliche Anforderungen unter-schiedliche Authentisierungsmechanismen gibt.
  ...
  * Die Teilnehmer verstehen den Begriff Idempotenz (speziell im Kontext von PUT vs. POST).
  ...
  * Die Teilnehmer verstehen die Auswirkungen von TLS für das Caching in Intermediaries.
...
**Was sollen die Teilnehmer kennen?**
  ...
  * Die Teilnehmer kennen den Unterschied zwischen Browser-Requests zur der Origin einer Webseite, also der Authority der in der Adresszeile stehenden URL, und zu anderen URLs (Cross-Domain Requests).
    ** Die Teilnehmer wissen, dass diese Unterschiedliche Behandlung in Form der Same-Origin Policy (SOP) umgesetzt wird. 
  * Die Teilnehmer kennen das Browser-intern verwendete Document Object Model (DOM) wissen, dass es durch JavaScript verändert werden kann.
  * Die Teilnehmer kennen die Aufgabe von CSS, welches die Darstellung der im DOM vorhandenen Strukturen und Daten beeinflusst.
  ...
  * Die Teilnehmer wissen, wie Cookies zwischen Client und Server ausgetauscht und verwaltet werden.
    ** 3rd Party Cookies nennt man Cookies, die von einer anderen Quelle als der origin gesetzt wurden.
    ** Das SameSite Cookie Attribut schützt Cookies, dass sie aus Sicherheitsgründen nicht 3rd Party werden können.
    ** Browser Tracking Prevention entscheidet, ob ein 3rd Party Cookie überhaupt gesendet wird.
  * Die Teilnehmer kennen die fetch API als Grundlage von AJAX-basierten Anwendungen.
  ...
  * Die Teilnehmer kennen die Möglichkeiten, die sich durch Server-Sent-Events ergeben.
...
* The participants understand the term idempotence (especially in the context of PUT vs. POST).
...
* The participants understand the effects of TLS on caching in intermediaries.
...
* The participants understand the difference between browser requests to the origin of a website, i.e., the authority of the URL shown in the address bar and requests to other URLs (cross-domain requests).
  ** The participants know that this different handling is implemented through the Same-Origin Policy (SOP).
* The participants know the Document Object Model (DOM) used internally in browsers and know that it can be modified by JavaScript.
* The participants know the role of CSS which influences the presentation of the DOM's structures and data. 
...
* The participants know how cookies are exchanged and managed between the client and server.
  ** Third-party cookies are cookies that are set by a source other than the origin.
  ** The SameSite cookie attribute prevents cookies from becoming third-party cookies for security reasons.
  ** Browser tracking prevention determines whether a third-party cookie is sent at all.
* The participants know the fetch API as the basis of AJAX-based applications.
...
* The participants know the possibilities that result from Server-sent events.
