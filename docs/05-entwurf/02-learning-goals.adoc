...
** JavaScript steuert das Verhalten, verleiht über Event-Handler Interaktivität und ermöglicht asynchrone Serverkommunikation (z.B. via AJAX, Server-Sent Events oder WebSockets).
...
* Die Teilnehmer verstehen, dass mit Microfrontends gemeint ist, dass eine Benutzeroberfläche nicht monolithisch, sondern aus mehreren, voneinander unabhängig bereitgestellten Teilen besteht, die von unterschiedlichen Teams oder Systemen entwickelt und ausgeliefert werden und erst im Frontend integriert werden (Frontend-Integration).
...
* Die Teilnehmer verstehen, dass browserseitige Anwendungen zusätzlichen Angriffsvektoren ausgesetzt sind, da der Ausführungsumgebung nicht vertraut werden kann.
...
* Die Teilnehmer kennen verschiedene Muster der Frontend-Integration, wie Links, Isolated Frames, Transclusion, Self-contained Components (in Form von Web Components) und Plug-in-Module (oft z.B. via Module Federation).
* Die Teilnehmer kennen Authentifizierungsmechanismen wie OpenID Connect oder SAML.
* Die Teilnehmer kennen Muster zum Umgang mit Tokens in frontendseitigen Anwendungen, da Tokens nur in absoluten Ausnahmefällen, z.B. im Session Storage, gehalten werden sollten.
...
** JavaScript controls the behavior, provides interactivity via event handlers, and enables asynchronous server communication (e.g. via AJAX, Server-sent events or WebSockets).
...
* The participants understand that microfrontends mean a user interface is not monolithic, but composed of multiple independently delivered parts, provided by different teams or systems and integrated at the frontend (Frontend integration).
...
* The participants understand that browser-side applications are exposed to additional attack vectors because the execution environment cannot be trusted.
...
* The participants know different frontend integration patterns, such as links, isolated frames, transclusion, self-contained components (in the form of Web Components), and plug-in modules (often, for example, via Module Federation).
* The participants know authentication mechanisms such as OpenID Connect or SAML.
* The participants know patterns for handling tokens in frontend-side applications, since tokens should only be stored in exceptional cases, for example in session storage.
